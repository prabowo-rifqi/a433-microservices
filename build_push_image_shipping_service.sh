...
# Build Docker image dengan tag "shipping-service:latest"
docker build -t $GITHUB_USERNAME/shipping-service:latest .

# Menambahkan tag baru ke image agar sesuai dengan format GitHub Container Registry (GHCR)
docker tag $GITHUB_USERNAME/shipping-service:latest ghcr.io/$GITHUB_USERNAME/a433-microservices/shipping-service:latest
...
docker push ghcr.io/$GITHUB_USERNAME/a433-microservices/shipping-service:latest